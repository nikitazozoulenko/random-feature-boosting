...
from models.xgboost_wrapper import XGBoostClassifierWrapper
...
def get_GradientRFRBoost_eval_fun(
    ...
    def evaluate_GRFRBoost(
        ...
        get_optuna_params = lambda trial : {
            ...
            "n_classes": trial.suggest_categorical("n_classes", [max(2, y.size(1))]),
...
def evaluate_LogisticRegression(
    ...
    get_optuna_params = lambda trial : {
        ...
        "l2_lambda": trial.suggest_float("l2_lambda", 1e-7, 0.1, log=True),
    }
...
def evaluate_End2End(
    ...
    get_optuna_params = lambda trial : {
        # Fixed values
        "in_dim": trial.suggest_categorical("in_dim", [X.size(1)]),
        "out_dim": trial.suggest_categorical("out_dim", [out_dim]),
        "loss": trial.suggest_categorical("loss", ["bce" if out_dim == 1 else "cce"]),
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs='+', 
        type=str, 
        default=["GradientRFRBoost", "LogisticRegression"], 
    ...
    parser.add_argument(
        ...
        nargs='+', 
        type=int, 
        default=[i for i in range(len(openML_cls_ids))], 
        help="List of datasets to run."
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/home/nikita/Code/random-feature-boosting/save/OpenMLClassification/",
...
from optuna_kfoldCV import run_all_openML_with_model, openML_cls_ids
...
if __name__ == "__main__":
    ...
    for model_name in args.models:
        ...
        if "End2End" in model_name:
            ...
            eval_fun = evaluate_LogisticRegression
        ...
        # GRFRBoost
            ...
            do_linesearch = False if "linesearchFalse" in model_name else True
            eval_fun = get_GradientRFRBoost_eval_fun("SWIM", upscale_type, activation, use_batchnorm, do_linesearch)
        else:
            raise ValueError(f"Model {model_name} not recognized.")

        # run the experiments
        run_all_openML_with_model(
            dataset_ids = openML_cls_ids[args.dataset_indices],
